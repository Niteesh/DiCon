...
public class DAO {
    ...
    public List<Map<String, Object>> getNewsFeed(int user_id, int latest_feed_id) {
        return db.queryForList("SELECT DISTINCT D.dp,D.fullname,T.text,T.timestamp,T.tweet_id,T.user_id " +
                " FROM twitter.tweets T INNER JOIN twitter.follows  F ON T.user_id = F.following_id OR T.user_id = ? " +
                ...
                "WHERE  (T.user_id = ? OR F.follower_id = ?) AND tweet_id > ? ORDER BY T.timestamp ASC;",user_id, user_id, user_id, latest_feed_id);
    }
